...
class Index:
    ...
    def __init__(self, name, db_passwd = 'root'):
        """
        ...
        self._repository_db = get_repository(self.name, 'mysql', db_passwd)
    ...
    def loadFromFile(cls, path='imsearch_index.tar.gz', name=None):
        ...
        db = get_repository(info['name'], 'mysql')
        db.clean()
    ...
    def saveIndex(self, path='imsearch_index.tar.gz'):
        ...
        with tarfile.open(path, 'w:gz') as tf:
            ...
            pkl_path = os.path.join('.','.imsearch','tmp', "{}.pkl".format(str(uuid.uuid4())))
            with open(pkl_path, 'wb') as temp:
                pickle.dump(data, temp)          #store the data to pkl path (temp path)
    ...
    def loadFromDB(self, save=False):
        '''
        Load images from DB for indexing
        '''
        #images = [(x[0],x[2]) for x in self._repository_db.dump()]
        images = [x[6] for x in self._repository_db.dump()]
        response = []
        for image_info in images:
            features = self._feature_extractor._extract(image_info, save=save)
            if features is None:
                response.append(False)
                continue
            reposiory_data = self._nmslib_index.addDataPoint(features)
            self._repository_db.update(reposiory_data)
            response.append(True)
        return response

    def addImage(self, image_path, save=False):
        """
        ...
        features = self._feature_extractor._extract(image_path, save=save)
        if features is None:
            return False
        reposiory_data = self._nmslib_index.addDataPoint(features)
    ...
    def addImageBatch(self, image_list, save=False):
        """
    ...
    def knnQuery(self, image_path, k=10, policy='global'):
        ...
        if not matches:
            ...
            '''
            knn = self._nmslib_index.knnQuery(
                features['feature'], 'feature', k=k) # list[dict{id, distance}]
            matches = [(self._repository_db.find(
                {'feature_index': x[0]}, many=False), 1.0/(x[1] + EPSILON)) for x in knn]
        return matches[:k]

    def knnQuery_from_img(self, image, k=10, policy='global'):
        """
        Query the index to search for k-nearest images in the database.
        Parameters
        ---------
        ...
        features = self._feature_extractor.extract_from_img(image, save=False)
        if features is None:
            return []
        matches = []
        if policy == 'object':
