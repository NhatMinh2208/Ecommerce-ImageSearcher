...
#REDIS_DB = redis.StrictRedis.from_url('redis://localhost:6379/0')
REDIS_DB = redis.StrictRedis.from_url('redis://localhost:6379/0')
...
def main():
    while True:
        ...
        for _q in _queue:
            ...
            _q = json.loads(_q.decode("utf-8"))
            img = utils.base64_decode(_q['image'], _q['shape'])
            feature = extract_features(img.copy())
            result['feature'] = feature
            #result['feature'] = 'hello'
            #REDIS_DB.set(_q['id'], json.dumps(feature))
            REDIS_DB.set(_q['id'], json.dumps(result))
        REDIS_DB.ltrim(REDIS_QUEUE, len(_queue), -1)
