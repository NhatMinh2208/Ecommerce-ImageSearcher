...
def download_files():
    #path = os.path.join(os.environ.get('HOME'), '.imsearch', 'yolo')
    path = os.path.join(os.getcwd(), '.imsearch', 'yolo')
    os.makedirs(path, exist_ok=True)
    config_path = os.path.join(path, 'yolov3.cfg')
    if not os.path.exists(config_path):
        wget.download(
            'https://www.dropbox.com/s/9kacdlcdo717i1g/yolov3.cfg?dl=1', out=config_path)

    weights_path = os.path.join(path, 'yolov3.weights')
    if not os.path.exists(weights_path):
        wget.download(
            'https://www.dropbox.com/s/8ftd60s7z77ltv4/yolov3.weights?dl=1', out=weights_path)

    class_path = os.path.join(path, 'coco.names')
    if not os.path.exists(class_path):
        wget.download(
            'https://www.dropbox.com/s/96o871of6g0secz/coco.names?dl=1', out=class_path)
